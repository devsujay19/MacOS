<script lang="ts">
  ...
  const { needRefresh, updateServiceWorker } = useRegisterSW({
    ...
    onRegisterError(error) {
      console.log('Service Worker registration has an error! : ', error);
    },
  });
...
<SystemDialog bind:this={systemUpdateDialog}>
  <section class="system-update-section">
    <img
      ...
      alt="AppStore for MacOS"
      draggable="false"
    />

    <h3>Newest Updates are Available now!</h3>
    <p>Do you want to restart to install these updates now?</p>

    <div class="buttons">
      <button on:click={close}>Maybe later</button>
      <button class="confirm" on:click={handleUpdateApp}> Update now </button>
    </div>
  </section>
</SystemDialog>
